...
import { getDriverByNumberAndSeason } from "@/lib/api/drivers/queries";
...
const sliceRaces = raceArr.slice(0, 11);
...
sliceRaces.forEach(async (race: any, i: number) => {
  setTimeout(async () => {
    ...
    await insertResultsByRaceId(race.id, createdRace.id, 2024);
...
async function insertResultsByRaceId(raceId: string, dbId: string, season: number) {
  const options = {
    params: { race: raceId },
    headers: {
  ...
  results.forEach(async (result: any) => {
    let { driver } = await getDriverByNumberAndSeason(result.driver.number, season);
    if (!driver) {
